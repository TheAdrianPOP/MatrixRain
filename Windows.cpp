...
string chars_basic = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+=-`~[]\\{}|;':\",./<>?";
...
void resizeConsole(deque<rain>& rains) {
    ...
    if (!GetConsoleScreenBufferInfo(hOut, &csbi)) {
        throw runtime_error("Error getting console screen buffer info on windows");
    }
...
#else
bool kbhit_nonblock() { return false; }
int getch_nonblock() { return 0; }
void resizeConsole(deque<rain>& rains) {}
#endif
...
int main() {
    ...
    if (!SetConsoleMode(hOut, dwMode | ENABLE_VIRTUAL_TERMINAL_PROCESSING)) {
        fprintf(stderr, "Error setting console mode for ANSI escape codes.\n");
        return 1;
    }
#endif
    try {
        ...
        while (true) {
            ...
            if (!SetConsoleCursorPosition(hConsole, coord)) {
                throw runtime_error("Error setting console cursor position on windows");
            }
#else
            printf("\x1b[H");
#endif
            printf("%s", render(rains, commandLine, showHelp, showList, listText).c_str());
